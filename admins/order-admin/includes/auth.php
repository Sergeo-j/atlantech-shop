 ...
 * Atlantech Shop - Order Admin Dashboard
...
function secure_session_start() {
    ...
    $session_name = 'atlantech_order_admin';
...
// Vérifier si l'utilisateur est connecté ET a bien le rôle 'order'
...
function is_logged_in() {
    ...
    try {
        $st = $pdo->prepare("
            ...
            WHERE a.id = ? AND ar.role_name = 'order' AND a.is_active = 1
            LIMIT 1
        ");
        $st->execute([(int)$_SESSION['admin_id']]);
        return (bool)$st->fetchColumn();
    } catch (\Throwable $e) {
        error_log('order-admin is_logged_in: ' . $e->getMessage());
        return false;
    }
}
...
function login_admin($email, $password) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE a.email = ? AND ar.role_name = 'order'
            LIMIT 1
        ");
        $stmt->execute([$email]);
        $admin = $stmt->fetch();
        ...
        if (!$admin) {
            error_log("Order Admin non trouvé: " . $email);
            return false;
        }
        ...
        if (password_verify($password, $admin['password'])) {
            ...
            // Logger la connexion
            log_admin_action($admin['id'], 'LOGIN', 'Connexion au Order Admin Dashboard - ' . $admin['role_name']);
...
function logout_admin() {
    ...
    if (isset($_SESSION['admin_id'])) {
        log_admin_action($_SESSION['admin_id'], 'LOGOUT', 'Déconnexion du Order Admin Dashboard');
    }
