...
// auth.php ouvre la session avec le bon nom (atlantech_stock_admin)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        try {
            // Filtre strict : seul un admin avec le rôle 'stock' peut se connecter ici
            $stmt = $pdo->prepare("
                ...
                WHERE a.email = ? AND ar.role_name = 'stock'
                LIMIT 1
            ");
            $stmt->execute([$email]);
            $admin = $stmt->fetch();
            if ($admin && password_verify($password, $admin['password'])) {
                ...
                // Message générique pour ne pas révéler si c'est l'email/mdp ou le rôle qui manque
...
<head>
    ...
    <title>Connexion - Stock Admin</title>
