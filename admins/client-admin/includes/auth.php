 ...
 * AVEC SUPPORT NOUVELLE STRUCTURE DB
 */
...
function secure_session_start() {
    ...
    $session_name = 'atlantech_client_admin';
...
// Vérifier si l'utilisateur est connecté ET a bien le rôle 'client'
...
function is_logged_in() {
    ...
    try {
        $st = $pdo->prepare("
            ...
            WHERE a.id = ? AND ar.role_name = 'client' AND a.is_active = 1
            LIMIT 1
        ");
        $st->execute([(int)$_SESSION['admin_id']]);
        return (bool)$st->fetchColumn();
    } catch (\Throwable $e) {
        error_log('client-admin is_logged_in: ' . $e->getMessage());
        return false;
    }
}
...
function login_admin($email, $password) {
    ...
    try {
        // Filtre strict : seul un admin avec le rôle 'client' peut se connecter ici
        $stmt = $pdo->prepare("
            ...
            WHERE a.email = ? AND ar.role_name = 'client'
            LIMIT 1
        ");
        $stmt->execute([$email]);
        $admin = $stmt->fetch();
        ...
        if (!$admin) {
            error_log("Admin non trouvé: " . $email);
            return false;
        }
        ...
        if (password_verify($password, $admin['password'])) {
            ...
            // Logger la connexion
            log_admin_action($admin['id'], 'LOGIN', 'Connexion au dashboard Admin - ' . $admin['role_name']);
